...
def infer_type(value: str):
    ...
    if "," in value:
        ...
        list_items = []
        for item in items:
            parsed_item = infer_type(item).value  # 遞迴解析列表項目
            list_items.append(ast.Constant(value=parsed_item))
        return ast.List(elts=list_items, ctx=ast.Load())  # type: ignore
...
def gen_ini(config: configparser.ConfigParser):
    root_node = ast.Module(body=[], type_ignores=[])
    ...
    for section in config.keys():
        ...
        section_cls_node = ast.ClassDef(
            section, body=[], decorator_list=[], bases=[], keywords=[], type_params=[]
        )
        ...
        # root 內有section
        root_node.body.append(section_cls_node)

    strong_tree = ast.fix_missing_locations(root_node)
...
# print(example_ini.general.name)
